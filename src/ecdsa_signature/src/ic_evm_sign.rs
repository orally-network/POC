...
use crate::state::*;
use crate::ecdsa::reply::*;
use crate::ecdsa::request::*;
...
pub async fn create_address(principal_id: Principal) -> Result<CreateAddressResponse, String> {
    ...
    let caller = utils::get_derivation_path(principal_id);
    ...
    let address = utils::get_address_from_public_key(res.public_key.clone()).unwrap();
...
pub fn get_caller_data(principal_id: Principal, chain_id: u64) -> Option<UserResponse> {
    ...
    let address = utils::get_address_from_public_key(user.public_key.clone()).unwrap();
